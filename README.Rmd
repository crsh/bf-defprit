...
van Doorn et al. (in prep). Bayes Factors for Mixed Models: Perspective on Responses. *Computational Brain & Behavior*.
...
This project contains three seperate but interdependent pipelines

### Partial aggregation simulation

```{r echo = FALSE, results = "asis"}
cat("```mermaid")
targets::tar_mermaid(store = "_targets_partial_aggregation") |> 
  cat(sep = "\n")
cat("\n```")
```
...
```{r echo = FALSE, results = "asis"}
cat("```mermaid")
targets::tar_mermaid(store = "_targets_full_aggregation") |> 
  cat(sep = "\n")
cat("\n```")
```
...
```{r echo = FALSE, results = "asis"}
cat("```mermaid")
targets::tar_mermaid(store = "_targets_report") |> 
  cat(sep = "\n")
cat("\n```")
```
